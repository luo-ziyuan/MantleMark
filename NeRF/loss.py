...
class NeRFLoss_freq(nn.Module):
    ...
    def forward(self, pred, target):
        rgb_results = pred.permute(0, 3, 1, 2)
        target = target.permute(0, 3, 1, 2)
        target = 1-target
        fft_results_abs = torch.abs(torch.fft.fftshift(torch.fft.fft2(rgb_results, dim=(-2, -1))))
        threshold = 1.0
        zero_loss = torch.mean((fft_results_abs * (1-target))**2)
        non_zero_loss = torch.mean((torch.maximum(threshold - (fft_results_abs * target), torch.tensor(0.0))) ** 2)
