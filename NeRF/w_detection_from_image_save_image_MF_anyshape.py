...
def read_setting(filepath):
    ...
    shape = settings.get('shape', 'ring')
    return settings['mean_dis'], settings['min_f'], settings['max_f'], shape
...
def create_cube_template(image_shape, pose, mean_dis, min_f, max_f):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            template[i, j] = watermark_function_cube_shell_smooth(point_3d, min_f_scaled, max_f_scaled)
...
def create_tetrahedron_template(image_shape, pose, mean_dis, min_f, max_f):
    rows, cols = image_shape
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            template[i, j] = watermark_function_tetrahedron_shell_smooth(point_3d, min_f_scaled, max_f_scaled)
...
def create_octahedron_template(image_shape, pose, mean_dis, min_f, max_f):
    rows, cols = image_shape
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            template[i, j] = watermark_function_octahedron_shell_smooth(point_3d, min_f_scaled, max_f_scaled)
...
def read_images(original_results, watermarked_results, watermarked_dataset, n_test, distortion_args=None, save_images=False, detector='band_zero'):
    from sklearn import metrics
    mean_dis, min_f, max_f, shape = read_setting(os.path.join(watermarked_dataset, "setting.json"))
    transforms = read_json(os.path.join(watermarked_dataset, "transforms_test_watermark.json"))
    
    print(f"Using watermark shape: {shape}")
    print(f"Frequency range: {min_f} - {max_f}")
    print(f"Mean distance: {mean_dis}")
    ...
    # Process images
    for i in tqdm(range(n_test), desc=f"Processing images with {shape} watermark"):
        file_suffix = f"ngp_{i:06d}_rgb"
        ...
        process_image_pair(original_image, watermarked_image, pose, mean_dis, min_f, max_f, shape, detector,
    ...
    for condition in all_results:
        ...
        # Print results
        print(f"\nResults for {condition} ({shape} watermark):")
    ...
    metrics_save_path = os.path.join(watermarked_results, f"metrics_results_with_distortions.json")
    with open(metrics_save_path, 'w') as f:
        json.dump(metrics_summary, f, indent=4)

    # Save raw data (original_means and watermarked_means) for all conditions
    raw_data_summary = {}
    for condition in all_results:
        raw_data_summary[condition] = {
            'original_means': {
                'R_channel': all_results[condition]['original_means'][0],
                'G_channel': all_results[condition]['original_means'][1], 
                'B_channel': all_results[condition]['original_means'][2],
                'Mean_channels': all_results[condition]['original_means'][3]
            },
            'watermarked_means': {
                'R_channel': all_results[condition]['watermarked_means'][0],
                'G_channel': all_results[condition]['watermarked_means'][1],
                'B_channel': all_results[condition]['watermarked_means'][2], 
                'Mean_channels': all_results[condition]['watermarked_means'][3]
            }
        }
    
    raw_data_save_path = os.path.join(watermarked_results, f"raw_detection_data.json")
    with open(raw_data_save_path, 'w') as f:
        json.dump(raw_data_summary, f, indent=4)
    
    print(f"\nMetrics summary saved to: {metrics_save_path}")
    print(f"Raw detection data saved to: {raw_data_save_path}")
...
def process_image_pair(original_image, watermarked_image, pose, mean_dis, min_f, max_f, shape, detector, original_means, watermarked_means):
    ...
    if shape == 'ring':
        ...
        template = create_cube_template(original_data.shape[:2], pose, mean_dis, min_f, max_f)
    elif shape == 'tetrahedron':
        template = create_tetrahedron_template(original_data.shape[:2], pose, mean_dis, min_f, max_f)
    elif shape == 'octahedron':
        template = create_octahedron_template(original_data.shape[:2], pose, mean_dis, min_f, max_f)
    else:
        raise ValueError(f"Unsupported shape: {shape}. Use 'ring', 'cube', 'tetrahedron' or 'octahedron'.")
