...
class SSIMMeter:
    ...
    def write_json(self, file_path):
        if os.path.exists(file_path):
            ...
            data = {}
        result = self.measure().item()
        
        data.update({"SSIM": result})
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, staged=False, bg_color=bg_color, perturb=False, force_all_rays= True, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        outputs = self.model.render(rays_o, rays_d, staged=True, bg_color=bg_color, perturb=perturb, force_all_rays=True, **vars(self.opt))
    ...
    def plot_and_save_spectrum(self, image, save_path=None):
        _, _, c = image.shape
        ...
        if save_path:
            save_path_fft_npy = f'{os.path.splitext(save_path)[0]}.npy'
            np.save(save_path_fft_npy, fft_all_channels)

#         return f_transform_all_ch
    ### ------------------------------
...
#                 self.evaluate_train_set_one_epoch(train_loader)
#                 self.evaluate_one_epoch(valid_loader)
    ...
    def mask_with_pose(pose, mean_dis, min_f, max_f):
    ...
    def test(self, loader, save_path=None, name=None, write_video=False):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                pose = data['poses']
#                 mask = mask_with_pose(pose, self.opt.mean_dis, self.opt.min_f, self.opt.max_f)
                with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                save_path_rgb = os.path.join(save_path, 'test', f'{name}_{i:06d}_rgb.png')
                save_path_depth = os.path.join(save_path, 'test', f'{name}_{i:06d}_depth.png')
                save_path_fft = os.path.join(save_path, 'test', f'{name}_{i:06d}_fft.png')
                #self.log(f"==> Saving validation image to {save_path}")
                os.makedirs(os.path.dirname(save_path_rgb), exist_ok=True)

                cv2.imwrite(save_path_rgb, cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
                cv2.imwrite(save_path_depth, pred_depth)
                self.plot_and_save_spectrum(pred, save_path_fft)
    ...
    def evaluate_train_set_one_epoch(self, loader, name=None):
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    ...
                    # save image
                    save_path = os.path.join(self.workspace, 'validation_train_set', f'{name}_{self.local_step:04d}_rgb.png')
                    save_path_depth = os.path.join(self.workspace, 'validation_train_set', f'{name}_{self.local_step:04d}_depth.png')
                    save_path_fft = os.path.join(self.workspace, 'validation_train_set', f'{name}_{self.local_step:04d}_fft.png')
                    #self.log(f"==> Saving validation image to {save_path}")
                    os.makedirs(os.path.dirname(save_path), exist_ok=True)
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    ...
                    save_path_fft = os.path.join(self.workspace, 'validation', f'{name}_{self.local_step:04d}_fft.png')
                    #self.log(f"==> Saving validation image to {save_path}")
                    os.makedirs(os.path.dirname(save_path), exist_ok=True)
